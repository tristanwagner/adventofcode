...
const findShortestDistance = (intersections) => {
  const distances = intersections.map(intersection => intersection.distance)
  return Math.min(...distances)
}

const findShortestSteps = (intersections) => {
  const steps = intersections.map(intersection => intersection.steps)
  return Math.min(...steps)
}

const getIntersections = (firstPath, secondPath) => {
  const intersections = []
  for (const pointa of firstPath) {
    for (const pointb of secondPath) {
      if (pointa.x === pointb.x && pointa.y === pointb.y && pointa.steps && pointb.steps) {
        intersections.push({ x: pointa.x, y: pointa.y, steps: pointa.steps + pointb.steps, distance: Math.abs(pointa.x) + Math.abs(pointb.y) })
      }
    }
  }
...
console.time('firstWire')
const firstWirePath = computeInstructions(firstWire)
console.timeEnd('firstWire')

console.time('secondWire')
const secondWirePath = computeInstructions(secondWire)
console.timeEnd('secondWire')

console.time('intersections')
const intersections = getIntersections(firstWirePath, secondWirePath)
console.timeEnd('intersections')

console.log('intersections', intersections)

console.log('first answer', findShortestDistance(intersections))
console.log('second answer', findShortestSteps(intersections))
